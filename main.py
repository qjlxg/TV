...
def main():
    ...
    def check_url(url, channel_name, timeout=6):
        ...
        try:
            ...
            logging.warning(f"检测错误 {channel_name}: {url}: {e}")
    ...
    def process_urls_multithreaded(lines, max_workers=200):
        ...
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            # 使用 tqdm 包装 futures 迭代器以显示进度条
            futures = {executor.submit(process_line, line): line for line in lines}
            for future in tqdm(as_completed(futures), total=len(lines), desc="正在检测频道有效性"):
